...
class SchemaFaker:
    ...
    def nearbyPoint(self, long, lat, radius = (500, 2000)):
        rad = self.ff.random.uniform(radius[0], radius[1]) / 111000
        ang = 2*pi*self.ff.random.random()
        long1 = round(long + rad * cos(ang), 5)
        lat1 = round(lat + rad * sin(ang), 5)
        return long1, lat1

    def randPlace(self, country_code='US', city=''):
        all_places = geo.Provider.land_coords

        filtered_places = [x for x in all_places if ((x[3] == country_code or country_code == '')
                                             and (x[4].__contains__(city)))]
        return self.ff.random_element(filtered_places)
    ...
    def genRandomSchema(self):
        ...
        venue["lat"], venue["lon"] = self.nearbyPoint(float(place[0]), float(place[1]), radius=[100, 500])
